...
class t_BDPC_SRC6(Base_Script):
    
    test_info = {
        'name': "BDPC SRC6 Sensor Calibration",
        'version': 1.0
    }
    ...
    def open(self):
        ...
        try:
            conv = self.instr.findInstrument(resID='COM7')[0] # COM15 for the nice RS-485 converters
            conv.loadDriver('UPEL.BDPC.m_BDPC_SRC6')
            self.logger.debug("Registered BDPC SRC6")
        ...
        # Instruments
        self.requireInstrument('Secondary Voltage Measure', 'sec_voltage', deviceVendor='BK Precision', deviceType='Multimeter') #'123E12681')
        self.requireInstrument('Source - Secondary', 'sec_source', serial='00257')
        self.requireInstrument('BDPC SRC6', 'conv', driver='UPEL.BDPC.m_BDPC_SRC6')
        ...
        # self.requireInstrument('Load - Primary', 'pri_load', driver='models.BK_Precision.Load.m_85XX')
        # self.requireInstrument('Load - Secondary', 'sec_load', driver='models.BK_Precision.Load.m_85XX')
    ...
    def startup(self):
        try:
            ...
            status = self.conv.getStatus()

            #Get the name of the board and primary/secondary side of each board
            global ID
            ID = self.input('Enter the name of the board: ')
            global boardSide
            boardSide = self.input('Enter the side of the board(primary or secondary): ')
    ...
    def shutdown(self):
        # turn off so there's no voltage present when disconnecting/connecting module
        self.conv.stopOption('Switching')
    ...
    def writeCalibration(self, calibrationData):
        ...
        try:
            ...
            writer.writerow(ID, boardSide, 'Gain: ',calibrationData[0], 'Offset: ',calibrationData[1])
    ...
    def test_CalibrateSecVoltage(self):        
        ...
        self.writeCalibration(res)

        #Set secondary offset and gain voltage register with value
        self.setSensorGain(SecondaryVoltage, z[0])
        self.setsensorOffset(SecondaryVoltage, z[1])            
        return True
...
#===============================================================================
    ...
    def cal_SecVoltage(self):
        ...
        for opPoint in steps:
            ...
            for desired_x in range(num_dataPoints):
                try:
                    _, dataPoint_m, _, _, _ = self.conv.getOutputVoltage()#Change out all of the things that are not part of BDPC e.g. all with conv.....
                except:
                    status = self.conv.getStatus()
                    self.logger.error("Converter reported status: %s", status)
    ...
    def cal_PriVoltage(self):
        ...
        for opPoint in steps:
            ...
            for desired_x in range(num_dataPoints):
                _, _, dataPoint_m, _, _ = self.conv.getOutputVoltage()
                sampleIndex += 1
        ...
        #Set secondary offset and gain voltage register with value
        self.setSensorGain(PrimaryVoltage, z[0])
        self.setsensorOffset(PrimaryVoltage, z[1])
    ...
    def cal_CurrentSensor(self):
        ...
        for opPoint in steps:
            ...
            for desired_x in range(num_dataPoints):
                _, _, _, dataPoint_m, _ = self.conv.getOutputVoltage()
                sampleIndex += 1
    ...
    def test_ClosedLoopRegulation(self):
        ...
        while (points > 0):
            try:	
                dataPoint = self.sec_voltage.getMeasurement()
                _, vout, _, _, status = self.conv.getOutputVoltage()
